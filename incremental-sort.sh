	...
	for mworkers in 0 2; do
		...
		for wm in 4MB 8MB 16MB 32MB 64MB 128MB 256MB; do
			...
			for incremental in on off; do
				...
				sql="SELECT * FROM (SELECT * FROM s_1 ORDER BY a) foo ORDER BY a, b"

				d=`date`

				echo "===== $ID [$d] $SCALE $ngroups $wm $incremental $mworkers =====" >> $OUT/explains.log
				echo "$sql" >> $OUT/explains.log
				psql test -c "EXPLAIN $sql" >> $OUT/explains.log
...
EOF
				...
				sql="SELECT * FROM (SELECT * FROM s_2 ORDER BY a, b) foo ORDER BY a, b, c"
...
EOF
				...
				sql="SELECT * FROM (SELECT * FROM s_3 ORDER BY a, b, c) foo ORDER BY a, b, c, d"
...
EOF
				...
				sql="SELECT * FROM (SELECT * FROM s_4 ORDER BY a, b, c, d) foo ORDER BY a, b, c, d, e"
